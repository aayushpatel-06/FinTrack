...
function App() {
  ...
  const saveExpense = async (e) => {
    ...
    try {
      if (editingId) {
        await updateDoc(doc(db, 'expenses', editingId), { amount: parseFloat(amount), category, isNeed });
        setEditingId(null);
      } else {
        await addDoc(collection(db, 'expenses'), { uid: user.uid, amount: parseFloat(amount), category, isNeed, createdAt: serverTimestamp() });
      }
      setAmount('');
  ...
  const chartData = [
    { name: 'Needs', value: expenses.filter(e => e.isNeed).reduce((s, i) => s + i.amount, 0) || 0.01 },
    { name: 'Wants', value: expenses.filter(e => !e.isNeed).reduce((s, i) => s + i.amount, 0) || 0.01 },
  ];
  ...
  if (!user) {
    return (
      <div className="min-h-screen w-full flex items-center justify-center bg-gradient-to-br from-blue-600 via-indigo-700 to-purple-800 p-6">
        <div className="w-full max-w-xl bg-white/95 backdrop-blur-xl rounded-[4rem] p-12 sm:p-20 shadow-2xl text-center">
      ...
      <main className="flex-1 w-full px-6 sm:px-12 py-10 space-y-10">
        ...
        <div className="flex flex-col xl:flex-row gap-8">
          <div className={`relative overflow-hidden flex-1 p-12 rounded-[4rem] text-white shadow-2xl transition-all duration-700 ${percentageUsed > 80 ? 'bg-rose-500' : 'bg-blue-600'}`}>
            ...
            <div className="relative z-10 w-full bg-white/20 rounded-full h-5 p-1.5 shadow-inner">
              <div className="bg-white h-full rounded-full transition-all duration-1000 shadow-[0_0_20px_white]" style={{ width: `${Math.min(percentageUsed, 100)}%` }}></div>
            </div>
            <div className="relative z-10 flex justify-between mt-8 items-center">
               ...
               {percentageUsed > 80 ? <TrendingUp size={48} className="text-white animate-bounce" /> : <TrendingDown size={48} className="text-emerald-300" />}
            </div>
          </div>
            ...
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 group">
              <div className="flex items-center gap-5">
                ...
                <div className="flex-1">
                  ...
                  <input id="budget-input" name="monthlyLimit" type="number" value={monthlyBudget} onChange={(e) => setMonthlyBudget(e.target.value)} className="bg-transparent font-black text-4xl w-full focus:outline-none text-gray-900" />
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <form onSubmit={saveExpense} className="bg-white/80 backdrop-blur-xl border border-white p-10 sm:p-14 rounded-[4.5rem] shadow-2xl space-y-10">
            <div className="flex flex-col gap-3">
               <label htmlFor="exp-amount" className="text-xs font-black text-blue-600 ml-6 uppercase tracking-[0.4em]">Transaction Amount</label>
               <input 
                 id="exp-amount"
                 name="expenseAmount"
                 type="number" 
                 ...
                 inputMode="decimal"
                 className="w-full p-10 rounded-[3rem] bg-gray-50 border-none ring-4 ring-gray-100 focus:ring-12 focus:ring-blue-50 text-6xl font-black transition-all outline-none text-gray-900" 
               />
            </div>
            ...
            <div className="flex flex-col md:flex-row gap-6">
              <div className="flex-1 flex flex-col gap-2">
                ...
                <select id="cat-select" name="expenseCategory" value={category} onChange={(e) => setCategory(e.target.value)} className="w-full p-8 rounded-[2.5rem] bg-gray-50 border-none ring-4 ring-gray-100 font-black text-2xl cursor-pointer outline-none appearance-none">
            ...
            <div className="flex gap-12 px-6">
              <label className="flex items-center gap-4 text-xl font-bold text-gray-600 cursor-pointer group">
          ...
          {/* PERMANENT FIX FOR RECHARTS DIMENSION ERROR */}
              ...
              {mounted && (
                <ResponsiveContainer width="100%" height="100%" debounce={100}>
                  <PieChart margin={{ top: 0, right: 0, bottom: 0, left: 0 }}>
                    <Pie data={chartData} cx="50%" cy="50%" innerRadius={90} outerRadius={125} paddingAngle={12} dataKey="value" isAnimationActive={true} nameKey="name">
                      {chartData.map((e, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} cornerRadius={25} stroke="none" />)}
                    </Pie>
        ...
        <div className="w-full pb-20">
          <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest mb-10 px-8 text-left">Activity Feed</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 2xl:grid-cols-4 gap-8">
            {expenses.map(exp => (
              <div key={exp.id} className="bg-white/80 backdrop-blur-md border border-white p-10 rounded-[3.5rem] shadow-lg hover:-translate-y-2 transition-all flex justify-between items-center group">
                <div className="flex items-center gap-6">
                  <div className={`w-3 h-3 rounded-full ${exp.isNeed ? 'bg-emerald-500 shadow-[0_0_15px_#10b981]' : 'bg-rose-500 shadow-[0_0_15px_#f43f5e]'}`}></div>
                  <div className="text-left">
                    <p className="font-black text-2xl text-gray-800 tracking-tighter">{exp.category}</p>
                    <p className="text-[10px] font-black text-gray-300 uppercase tracking-widest">
                       {exp.createdAt ? exp.createdAt.toDate().toLocaleDateString('en-IN') : 'Saving...'}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-6">
